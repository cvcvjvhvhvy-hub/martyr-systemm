...
class _AdminsPageState extends State<AdminsPage> {
  ...
  Future<void> _showEditor({AdminAccount? a}) async {
    ...
    final perms =
        TextEditingController(text: a != null ? (a.permissions.join(',')) : '');
    final ok = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(
              controller: name, decoration: InputDecoration(hintText: 'الاسم')),
          TextField(
              controller: email,
              decoration: InputDecoration(hintText: 'البريد')),
          TextField(
              controller: roleCtrl,
              decoration: InputDecoration(hintText: 'الدور')),
          TextField(
              controller: perms,
              decoration:
                  InputDecoration(hintText: 'الصلاحيات (مفصولة بفاصلة)')),
        ]),
        actions: [
          TextButton(
              ...
              child: Text('إلغاء')),
          ElevatedButton(
              onPressed: () => Navigator.pop(context, true), child: Text('حفظ'))
        ],
      ),
    );
    if (ok != true) return;
    if (a == null) {
      final na = AdminAccount(
          id: Uuid().v4(),
          name: name.text.trim(),
          email: email.text.trim(),
          role: roleCtrl.text.trim(),
          permissions: perms.text.trim().isEmpty
              ? []
              : perms.text.split(',').map((s) => s.trim()).toList());
      await _svc.addAdmin(na);
      await _svc.recordAudit(AuditEntry(
          id: Uuid().v4(),
          adminId: _svc.currentAdminId(),
          action: 'create_admin',
          targetType: 'admin',
          targetId: na.id,
          details: na.role));
      ...
      a.permissions = perms.text.trim().isEmpty
          ? []
          : perms.text.split(',').map((s) => s.trim()).toList();
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (_, i) {
                              ...
                              return ListTile(
                                ...
                                subtitle: Text(
                                    '${it.adminId} • ${it.targetType}:${it.targetId} • ${it.at.toLocal()}'),
                              );
                            })),
                actions: [
                  TextButton(
  ...
  Future<void> _exportAudit(List<AuditEntry> list) async {
    ...
    for (final a in list)
      rows.add(
          '${a.id},${a.adminId},${a.action},${a.targetType},${a.targetId},"${a.at.toIso8601String()}","${a.details ?? ''}"');
    final csv = rows.join('\n');
    PlatformUtils.downloadCsv(csv, 'audit.csv');
    ScaffoldMessenger.of(context)
        .showSnackBar(SnackBar(content: Text('تم تنزيل/حفظ سجل التدقيق')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('حسابات الأدمن'), actions: [
        if (canViewAudit)
          IconButton(
              icon: Icon(Icons.history),
              onPressed: () => Navigator.of(context)
                  .push(MaterialPageRoute(builder: (_) => AuditPage())))
      ]),
      body: _loading
      ...
      floatingActionButton: canManage
          ? FloatingActionButton(
              onPressed: () => _showEditor(), child: Icon(Icons.add))
          : null,
    );
  }
}
